...
export function DeckSessionClient({
  ...
  const progressPercent = Math.max(0, Math.min(100, Math.round((answered / total) * 100)));
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 pb-4 border-b border-outline-variant/30 gap-4">
        ...
        <div className="flex items-center gap-4">
           <div className="flex gap-1 h-1.5 w-32 rounded-full overflow-hidden bg-secondary-container">
             <div className="h-full bg-success transition-all duration-300" style={{ width: `${progressPercent}%` }}></div>
           </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 flex-1">
        ...
        <div className="lg:col-span-8 flex flex-col gap-6">
          ...
          >
             ...
             <div className={`font-display text-center leading-tight text-foreground transition-all ${prompt.length > 6 ? 'text-6xl md:text-8xl' : 'text-8xl md:text-[160px]'}`}>
               {prompt}
             </div>
          ...
          {mode === "flashcard" && (
               ...
               {!revealed ? (
                 ...
                 <div className="flex justify-between gap-4 w-full">
                   {ratingButtons.map((btn) => (
                     ...
                     >
                       <span className="text-xs font-bold opacity-70 mb-1">{btn.key}</span>
           ...
           {/* Stroke Order Canvas (Decorative stub matching Image 2) */}
           <div className="hidden lg:flex w-full h-[320px] bg-surface-lowest rounded-[2rem] border-2 border-dashed border-outline-variant/30 relative items-center justify-center mb-8">
             <div className="absolute top-4 left-4 right-4 bottom-4 border border-outline-variant/10 rounded-xl"></div>
             <span className="font-display text-[200px] text-surface-low select-none pointer-events-none opacity-50 relative z-0">{prompt}</span>
             <div className="absolute bottom-6 bg-surface-lowest px-4 py-1.5 rounded-full z-10 text-[10px] font-bold text-primary uppercase tracking-widest shadow-sm border border-outline-variant/10">
               Stroke Order Canvas
             </div>
           </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-4">
          ...
          <h3 className="text-[10px] font-bold uppercase tracking-[0.2em] text-primary mb-2">Linked Vocabulary</h3>
          ...
          <div className="bg-[#e4f3ed] rounded-2xl p-5 flex items-center justify-between shadow-sm relative overflow-hidden group border border-[#2a9a8c]/10">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h4 className="font-bold text-foreground text-lg group-hover:text-success transition-colors">{prompt}曜日</h4>
                <p className="text-[10px] text-on-surface-variant font-bold opacity-70">Mokuyōbi • Thursday</p>
              </div>
            </div>
          ...
          <div className="bg-surface-lowest rounded-2xl p-5 flex items-center justify-between shadow-sm border border-outline-variant/5">
            <div className="flex items-center gap-4">
              <span className="text-secondary text-xs font-bold w-6 opacity-60">02</span>
              <div>
                <h4 className="font-bold text-foreground text-lg">大{prompt}</h4>
                <p className="text-[10px] text-on-surface-variant font-bold opacity-70">Taiki • Large tree</p>
              </div>
            </div>
          ...
          <div className="bg-surface-lowest rounded-2xl p-5 flex items-center justify-between shadow-sm border border-outline-variant/5">
            <div className="flex items-center gap-4">
              <span className="text-secondary text-xs font-bold w-6 opacity-60">03</span>
              <div>
                <h4 className="font-bold text-foreground text-lg">{prompt}材</h4>
                <p className="text-[10px] text-on-surface-variant font-bold opacity-70">Mokuzai • Lumber</p>
              </div>
            </div>
            <button className="w-8 h-8 rounded-full bg-surface-low flex items-center justify-center text-secondary hover:text-primary transition-colors focus:outline-none">
               <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M3 9v6h4l5 5V4L7 9H3zm13.5 3c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02z"/></svg>
          ...
          <div className="bg-[#001736] rounded-2xl p-6 shadow-lg text-white mt-4 relative overflow-hidden">
             ...
             <div className="relative z-10">
               <div className="flex items-center gap-2 mb-4">
                 <svg className="w-4 h-4 text-[#8ef4e4]" fill="currentColor" viewBox="0 0 24 24"><path d="M9 21c0 .5.4 1 1 1h4c.6 0 1-.5 1-1v-1H9v1zm3-19C8.1 2 5 5.1 5 9c0 2.4 1.2 4.5 3 5.7V17c0 .5.4 1 1 1h6c.6 0 1-.5 1-1v-2.3c1.8-1.3 3-3.4 3-5.7 0-3.9-3.1-7-7-7z"/></svg>
                 <span className="text-[10px] font-bold uppercase tracking-widest text-white/90">Mnemonics Tip</span>
               </div>
               <p className="text-sm text-white/80 leading-relaxed font-light mb-6">
                 Imagine the horizontal stroke as the ground and the vertical stroke with its branches as a growing tree. Simple and rooted.
               </p>
             </div>
          </div>
