    ...
    <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {lessons.map((lesson, index) => (
        <Link
          ...
          className="fade-in-up group relative overflow-hidden rounded-3xl border border-rose-900/15 bg-gradient-to-br from-white via-rose-50 to-orange-50 p-5 shadow-lg shadow-rose-950/5 transition duration-300 hover:-translate-y-1 hover:border-rose-700/30 hover:shadow-2xl"
          style={{ animationDelay: `${index * 70}ms` }}
        >
          <p className="text-xs uppercase tracking-[0.25em] text-rose-700">Deck</p>
          <h2 className="mt-3 font-display text-3xl text-slate-900">{lesson.title}</h2>
