        ...
        {isSignedIn && user ? (
          <Link
            ...
            className={`flex items-center gap-3 rounded-xl px-3 py-2.5 transition-all hover:bg-surface-low group ${
              pathname === "/profile" ? "bg-surface-low text-primary" : "text-on-surface-variant"
            }`}
          >
            {user.image ? (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={user.image} alt={user.name} className="h-8 w-8 rounded-full ring-2 ring-transparent group-hover:ring-primary/20 transition-all shrink-0" />
            ) : (
